...
// aionui 自带按钮 源码/预览/分屏 → 同款图标 (保留 onclick)
const AION_ICON_MAP = { '源码': 'code', '预览': 'eye', '分屏': 'columns' }
const AION_SVG = {
  code: '<polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/>',
  eye: '<path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7-11-7-11-7z"/><circle cx="12" cy="12" r="3"/>',
  columns: '<rect x="3" y="3" width="18" height="18" rx="2"/><line x1="12" y1="3" x2="12" y2="21"/>',
}
function iconifyAionButtons() {
  const col = document.querySelector('.aionui-preview-col')
  if (!col) return
  col.querySelectorAll('button').forEach((b) => {
    const txt = (b.textContent || '').trim()
    const iconName = AION_ICON_MAP[txt]
    if (!iconName || b.dataset.vbIconified) return
    b.dataset.vbIconified = '1'
    const label = txt
    b.textContent = ''
    b.innerHTML = vbIcon(iconName)
    b.style.cssText = (b.style.cssText || '') + ';display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;padding:0'
...
// ===== aionui 预览工具栏扩展 (v17) =====
let aionLastPath = null
let aionLastRoot = null
  ...
  try {
    ...
    window.fetch = function (...args) {
      try {
        ...
        if (String(u).includes('/aionui-panel/read') || String(u).includes('/aionui-panel/list')) {
          ...
          if (b && b.root && b.path) aionLastPath = aionLastRoot + '/' + String(b.path).replace(/^\/+/, '')
        }
      } catch (_) {}
...
// 兜底: 从当前激活 tab 的标题拼路径 (仅工作区根级文件; 新打开的文件由 fetch 拦截拿到完整路径)
function aionCurrentPath() {
  if (aionLastPath) return aionLastPath
  const tab = document.querySelector('[class*="tabActive"] [class*="tabTitle"], [class*="tabActive"]')
  const title = tab ? (tab.textContent || '').trim() : ''
  if (title && aionLastRoot && !title.includes('/')) return aionLastRoot + '/' + title
  return null
}
...
function installAionToolbar() {
  ...
  const ensure = () => {
    const col = document.querySelector('.aionui-preview-col')
    if (!col) return
    // 每次 tick 都先图标化 aionui 自带按钮 (aionui 重渲染会重建它们)
    iconifyAionButtons()
    if (col.querySelector('.vb-aion-ext')) return
    const refreshBtn = [...col.querySelectorAll('button')].find((x) => (x.getAttribute('title') || '').includes('刷新') || (x.textContent || '').trim() === '刷新')
    const hostEl = refreshBtn ? refreshBtn.parentElement : null
    ...
    wrap.appendChild(mkBtn('在访达显示', 'folder', () => {
      const p = aionCurrentPath()
      if (!p) return vbToast('先在左侧打开一个文件')
    ...
    wrap.appendChild(mkBtn('复制路径', 'clip', () => {
      const p = aionCurrentPath()
      if (!p) return vbToast('先在左侧打开一个文件')
      vbCopyText(p)
    }))
    wrap.appendChild(mkBtn('复制文件（访达里可粘贴）', 'copy', () => {
      const p = aionCurrentPath()
      if (!p) return vbToast('先在左侧打开一个文件')
...
module.exports = {
  ...
  apply(ctx) {
    ...
    slots.inject('tool.call.toolview', () => slots.register(
      { name: 'tool.call.toolview', key: 'read' },
      (props) => {
    ...
    function UsageCard() {
      ...
      return h(React.Fragment, null,
        ...
        !open ? null : state.kind === 'loading' ? h('div', {
          style: {
