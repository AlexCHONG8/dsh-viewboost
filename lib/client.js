...
// 始终以「当前激活 tab」为准: 查映射表 → root+标题兜底 → aionLastPath
function aionCurrentPath() {
  ...
  if (title) {
    const mapped = aionPathByTab[title]
    if (mapped) return mapped
    if (aionLastRoot && !title.includes('/')) return aionLastRoot + '/' + title
  }
  return aionLastPath || null
}
