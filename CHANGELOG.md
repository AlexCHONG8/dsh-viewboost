...
## [1.0.1] — 2026-08-15 — 复制路径 / 在访达显示 真修复

### Fixed
- **复制路径抓错 tab**：全文档查 `[class*="tabActive"]` 先命中 DSH 会话标签（如「对话」）→ 复制出 `<工作区>/对话` 这种不存在的路径。改为只在 `.aionui-preview-col` 内找激活 tab，优先 `tabTitle` 的 `title` 属性
- **目录浏览污染当前路径**：`/aionui-panel/list`（展开文件夹）不再写入「当前文件」，只有 `read`（真正打开文件）才更新
- **子目录文件定位**：子目录文件的 tab 标题就是工作区相对路径（如 `04_传动机构/xxx.md`），直接拼 root 即为候选（原守卫误伤）
- **在访达显示 ENOENT**：所有候选经 host `vb.stat` 校验（只认「存在且 isDirectory === false」）；全败时用 `vb.list` 从工作区 root BFS 按文件名定位（≤6 层 / ≤150 目录，跳过隐藏与 node_modules 等）；仍找不到诚实提示，不再把虚构路径交给 Finder
- **切 tab 复制错文件**：不再退回「上一个读过的文件」——激活 tab 绑定候选优先，BFS 定位次之，无激活 tab 才用最近 read
- **图片预览无按钮**：图片模式工具栏没有「刷新」按钮导致 4 按钮不注入 → 注入锚点链 刷新 → 下载 → 任一按钮
- 复制成功 toast 显示完整路径，当场可核对
